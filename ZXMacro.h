...
#define HELVETICANUEU @"HelveticaNeue"
...
#define FileS(name,type)           [[NSBundle mainBundle]pathForResource:name ofType:type]
...
// 计算X坐标
#define kCalculateX(VIEW) VIEW.frame.origin.x+VIEW.frame.size.width
#define kCalculateY(VIEW) VIEW.frame.origin.y+VIEW.frame.size.height
// 获取控件大小
#define kGetViewWidth(x) x.frame.size.width
#define kGetViewHeight(x) x.frame.size.height
